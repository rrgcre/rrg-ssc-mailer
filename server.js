...
function personBrief(p) { const em = personEmails(p), ph = personPhones(p); return p ? { id: p.id, name: p.name || '', firstName: personFirst(p), lastName: personLast(p), nickname: p.nickname || '', company: p.company || '', companyId: p.companyId || '', emails: em, phones: ph, email: preferredEmailOf(p), phone: preferredPhoneOf(p), type: p.type || '', tags: personTags(p), leadSource: p.leadSource || '', vip: !!p.vip, caution: !!p.caution, star: !!p.star, prefContact: Array.isArray(p.prefContact) ? p.prefContact : [], createdAt: p.createdAt || '', owner: p.by || '', lastContacted: p.lastContacted || '', hasPhoto: !!p.photoExt } : null; }
...
const TOOL_DEFS = [
  ...
  { file: 'rrg_questionnaire_queue.html', name: 'Seller Interviews' },
...
// answers builder in the Seller Interview's Run-Analysis action).
...
function _bodyLooksHtml(body){ return /<(p|br|div|ul|ol|li|b|i|u|a|strong|em|img|span|h[1-6]|table)\b|<\/(p|div|li|ul|ol|a)>/i.test(String(body||'')); }
function trackedEmailHtml(body, origin, token, sigHtml) {
  const htmlBody = _bodyLooksHtml(body) ? String(body) : _escHtmlBody(body).split('\n').join('<br>');
...
async function runAutomationStep(p, en, step) {
  ...
  try {
    await sendMailWL({ from: mailFrom(), to: to, subject: subject, text: (_bodyLooksHtml(body) ? htmlToText(body) : body), html: trackedEmailHtml(body, origin, tok), replyTo: en.replyTo || undefined });
    const now = new Date().toISOString();
...
// The legacy server-rendered /admin console has been retired. Everything it did
// -- user accounts, tool access, quick links, documents, valuation rules --
// now lives in the unified Settings window as standalone panels. Old bookmarks
// and the "Back to admin console" links across the panels land there.
app.get('/admin', requireAdmin, (req, res) => res.redirect('/rrg_settings.html'));

// ---- Read-side endpoints for the Settings panels. The retired /admin console
// server-rendered this data inline; the standalone panels fetch it instead. ----
app.get('/api/admin/users', requireAdmin, (req, res) => {
  try {
    const _u = auth.loadUsers();
    const _last = auth.lastLoginMap();
...
function seedBizSalesStages() {
  try {
    ...
    if (fs.existsSync(marker)) return;
    const all = loadPipelines();
    const names = ['Lead Received', 'Qualification Call', 'Seller Interview', 'Alignment & Agreement', 'Marketing', 'Prospecting - Active', 'Prospecting - Not Active', 'Offers', 'Due Diligence', 'Close'];
...
app.delete('/api/document/:kind/:id', (req, res) => {
...
app.get('/api/documents', (req, res) => {
  ...
  try {
    ...
    qsts.forEach(q => {
      ...
      const title = (q.business && q.business !== 'Business') ? q.business : (d.company || 'Seller Interview');
      out.push({ id: q.id, kind:'questionnaire', title: title, typeLabel:'Seller Interview',
...
app.get('/api/screening/:id/view', (req, res) => {
  ...
  const _bs = 'display:inline-flex;align-items:center;gap:7px;text-decoration:none;font:600 13px -apple-system,Segoe UI,Roboto,sans-serif;padding:10px 16px;border-radius:9px;box-shadow:0 6px 20px rgba(16,24,40,.18);cursor:pointer;border:1px solid #d7dde8;background:#fff;color:#1a2236';
  const _eurl = './seller_screening.html?screening=' + encodeURIComponent(s.id);
...
app.get('/api/questionnaire/:id/view', (req, res) => {
  ...
  const pct = (typeof s.completePct === 'number' ? s.completePct : (s.completed ? 100 : 0));
  const rec = { data: s.data || {}, name: (s.business && s.business !== 'Business') ? s.business : ((s.data && s.data.company) || 'Seller Interview'), market: s.market || '', rep: s.by || s.byUser || '', timestamp: s.createdAt || '', highlights: s.completed ? 'Complete' : ('In progress — ' + pct + '%') };
  let html = intakeViewHtml(rec, 'Seller Interview');
...
const GATEABLE_TOOLS = [
  ...
  { file: 'rrg_questionnaire_queue.html', name: 'Seller Interviews', cat: 'Sell-Side' },
