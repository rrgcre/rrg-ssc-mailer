...
(function(){
  ...
  function injectCss(){ if(document.getElementById('rrgaiwork-css')) return; var st=document.createElement('style'); st.id='rrgaiwork-css';
    ...
    +'.rrgaiwbox{background:#fff;border:1px solid #dbe0e9;border-radius:8px;padding:30px 34px 24px;max-width:440px;width:100%;text-align:center;box-shadow:0 20px 50px rgba(11,26,56,.28);position:relative;overflow:hidden;}'
    +'.rrgaiwbox:before{content:"";position:absolute;top:-45%;left:-45%;width:190%;height:190%;background:conic-gradient(from 0deg,#20334f,#5a3d9e,#2c5c8f,#20334f);opacity:.05;animation:rrgaispin 9s linear infinite;}'
    +'@keyframes rrgaispin{to{transform:rotate(360deg);}}'
    +'.rrgaiworb{width:58px;height:58px;border-radius:50%;margin:0 auto 16px;background:conic-gradient(from 0deg,#20334f,#5a3d9e,#2c5c8f,#20334f);animation:rrgaispin 1.5s linear infinite;position:relative;z-index:1;}'
    ...
    +'.rrgaiwtimer{font-variant-numeric:tabular-nums;font-weight:800;font-size:22px;color:#5a3d9e;margin-top:12px;position:relative;z-index:1;}'
    +'.rrgaiwcancel{margin-top:18px;background:#fff;border:1px solid #c4ccda;border-radius:3px;padding:8px 18px;font:inherit;font-size:13px;font-weight:600;color:#5a6478;cursor:pointer;position:relative;z-index:1;}'
    +'.rrgaiwbarwrap{margin-top:15px;position:relative;z-index:1;}'
    +'.rrgaiwbar{height:8px;border-radius:5px;background:#eef1f6;border:1px solid #e0e5ee;overflow:hidden;}'
    +'.rrgaiwbarfill{height:100%;width:0;background:linear-gradient(90deg,#20334f,#5a3d9e);border-radius:5px;transition:width .35s ease;}'
    +'.rrgaiwmeta{display:flex;justify-content:space-between;gap:12px;font-size:11.5px;color:#8a93a3;font-weight:700;margin-top:7px;font-variant-numeric:tabular-nums;}'
    +'.rrgaiworb.done{animation:none;background:#eaf5ef;border:2px solid #2f7a55;}'
    +'.rrgaiworb.done:after{content:"\\2713";color:#2f7a55;font-weight:900;font-size:26px;background:#eaf5ef;}'
    +'.rrgaiwsum{color:#2b3648;font-size:13px;margin-top:8px;line-height:1.55;position:relative;z-index:1;}'
    +'.rrgaiwok{margin-top:18px;background:#20334f;border:1px solid #20334f;border-radius:3px;padding:9px 24px;font:inherit;font-size:13px;font-weight:700;color:#fff;cursor:pointer;position:relative;z-index:1;}'
    ...
    +'.rrgaicfcancel{background:#fff;border:1px solid #c4ccda;border-radius:3px;padding:9px 16px;font:inherit;font-size:13px;font-weight:700;color:#5a6478;cursor:pointer;}'
    +'.rrgaicfgo{background:#20334f;border:1px solid #20334f;border-radius:3px;padding:9px 18px;font:inherit;font-size:13px;font-weight:700;color:#fff;cursor:pointer;}';
    document.head.appendChild(st); }
  var AIW={onCancel:null,onClose:null,t0:0,timer:null};
  function fmt(ms){ var s=Math.max(0,Math.floor(ms/1000)); var m=Math.floor(s/60); s=s%60; return m+':'+(s<10?'0':'')+s; }
  function g(id){ return document.getElementById(id); }
  function ensure(){ var o=document.getElementById('rrgaiwork'); if(o) return o; injectCss(); o=document.createElement('div'); o.className='rrgaiwork'; o.id='rrgaiwork'; o.hidden=true; o.innerHTML='<div class="rrgaiwbox"><div class="rrgaiworb" id="rrgaiworb"></div><div class="rrgaiwtitle" id="rrgaiwtitle">Working…</div><div class="rrgaiwmsg" id="rrgaiwmsg"></div><div class="rrgaiwbarwrap" id="rrgaiwbarwrap" hidden><div class="rrgaiwbar"><div class="rrgaiwbarfill" id="rrgaiwbarfill"></div></div><div class="rrgaiwmeta"><span id="rrgaiwcnt"></span><span id="rrgaiweta"></span></div></div><div class="rrgaiwtimer" id="rrgaiwtimer">0:00</div><button type="button" class="rrgaiwcancel" id="rrgaiwcancel">Cancel</button><button type="button" class="rrgaiwok" id="rrgaiwok" hidden>OK</button></div>'; document.body.appendChild(o); o.querySelector('#rrgaiwcancel').addEventListener('click',function(){ var f=AIW.onCancel; hide(); if(typeof f==="function"){try{f();}catch(e){}} }); o.querySelector('#rrgaiwok').addEventListener('click',function(){ var f=AIW.onClose; hide(); if(typeof f==="function"){try{f();}catch(e){}} }); return o; }
  function show(msg,onCancel){ ensure(); var orb=g('rrgaiworb'); if(orb) orb.classList.remove('done'); g('rrgaiwtitle').textContent='✨ '+assistant()+' is working…'; g('rrgaiwmsg').textContent=msg||''; g('rrgaiwbarwrap').hidden=true; g('rrgaiwbarfill').style.width='0'; AIW.onCancel=onCancel||null; AIW.onClose=null; var cb=g('rrgaiwcancel'); if(cb){ cb.style.display=''; cb.hidden=false; } var ok=g('rrgaiwok'); if(ok) ok.hidden=true; var tm=g('rrgaiwtimer'); if(tm) tm.hidden=false; AIW.t0=Date.now(); if(AIW.timer) clearInterval(AIW.timer); AIW.timer=setInterval(function(){ var t=g('rrgaiwtimer'); if(t) t.textContent=fmt(Date.now()-AIW.t0); },1000); g('rrgaiwtimer').textContent='0:00'; g('rrgaiwork').hidden=false; }
  function setMsg(msg){ var m=g('rrgaiwmsg'); if(m) m.textContent=msg||''; }
  function progress(done,total,opts){ ensure(); opts=opts||{}; done=+done||0; total=+total||0; var w=g('rrgaiwbarwrap'); if(w) w.hidden=false; var pct=total>0?Math.min(100,Math.round(done/total*100)):0; var fill=g('rrgaiwbarfill'); if(fill) fill.style.width=pct+'%'; var noun=opts.noun?(' '+opts.noun):''; var cnt=g('rrgaiwcnt'); if(cnt) cnt.textContent=done.toLocaleString()+' of '+total.toLocaleString()+noun; var eta=g('rrgaiweta'); if(eta){ var el=Date.now()-AIW.t0; if(done>0 && total>done && el>1500){ var rem=el*(total-done)/done; eta.textContent='~'+fmt(rem)+' left'; } else if(total>0 && done>=total){ eta.textContent='finishing…'; } else { eta.textContent=''; } } if(opts.msg!=null) setMsg(opts.msg); }
  function done(opts){ ensure(); opts=opts||{}; if(AIW.timer){ clearInterval(AIW.timer); AIW.timer=null; } var el=Date.now()-AIW.t0; var orb=g('rrgaiworb'); if(orb) orb.classList.add('done'); g('rrgaiwtitle').textContent=opts.title||'Done'; var took=(opts.tookText!=null)?opts.tookText:('Took '+fmt(el)); g('rrgaiwmsg').textContent=opts.summary||''; var fill=g('rrgaiwbarfill'); if(fill && !g('rrgaiwbarwrap').hidden) fill.style.width='100%'; var cnt=g('rrgaiwcnt'); if(cnt && !g('rrgaiwbarwrap').hidden){ } var eta=g('rrgaiweta'); if(eta) eta.textContent=''; var tm=g('rrgaiwtimer'); if(tm){ tm.hidden=false; tm.textContent=took; tm.style.fontSize='13px'; tm.style.color='#5f6a7d'; tm.style.fontWeight='700'; } var cb=g('rrgaiwcancel'); if(cb){ cb.style.display='none'; cb.hidden=true; } var ok=g('rrgaiwok'); if(ok){ ok.hidden=false; ok.textContent=opts.okText||'OK'; } AIW.onCancel=null; AIW.onClose=opts.onClose||null; g('rrgaiwork').hidden=false; }
  ...
  window.rrgAiWork={show:show,setMsg:setMsg,progress:progress,done:done,hide:hide};
