...
(function(){
  ...
  function injectCss(){ if(document.getElementById('rlf-css')) return; var st=document.createElement('style'); st.id='rlf-css';
    ...
    +'.rlf-badge{background:var(--red,#DA2B1F);color:#fff;font-size:11px;font-weight:800;border-radius:3px;padding:1px 6px;}'
  ...
  function create(cfg){
    ...
    function renderFly(){ ensureFly(); var head='<div class="rlf-flyhd">Saved segments</div>';
      ...
      fetch('/api/saved-searches?list='+encodeURIComponent(cfg.list),{credentials:'same-origin',cache:'no-store'}).then(function(r){return r.json();}).then(function(j){
        FLYSAVED=(j&&j.searches)||[]; var admin=cfg.isAdmin?cfg.isAdmin():false; var cur=norm(FILTERS); var FLYDEF=(j&&j.defaultId)||''; var rows;
        ...
        else { rows=FLYSAVED.map(function(sc,i){ var active=norm(sc.payload)===cur; var isDef=(FLYDEF===sc.id); var tag=sc.shared?('Shared'+(sc.mine?'':(' · '+esc(sc.ownerName||'team')))):'Just me'; return '<div class="rlf-flyrow'+(active?' act':'')+'" data-load="'+i+'"><span class="rlf-flychk">'+(active?'✓':'')+'</span><span class="rlf-flynm">'+esc(sc.name)+'<small>'+tag+(isDef?' · default':'')+'</small></span><button class="rlf-flydef" data-def="'+esc(sc.id)+'" title="'+(isDef?'Default view — click to clear':'Set as default view (auto-applies when you open this list)')+'" style="border:none;background:none;cursor:pointer;color:'+(isDef?'#e5a419':'#c1c8d6')+';font-size:14px;line-height:1;flex:none;padding:0 3px">'+(isDef?'★':'☆')+'</button>'+((sc.mine||admin)?'<button class="rlf-flyx" data-del="'+esc(sc.id)+'" title="Delete">✕</button>':'')+'</div>'; }).join(''); }
        ...
        FLY.querySelectorAll('[data-def]').forEach(function(b){ b.addEventListener('click',function(e){ e.stopPropagation(); var id=b.getAttribute('data-def'); var newId=(FLYDEF===id)?'':id; fetch('/api/saved-searches/default',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({list:cfg.list,id:newId})}).then(function(r){return r.json();}).then(function(){ renderFly(); }).catch(function(){}); }); });
    ...
    mkSavedBtn();
    // Auto-apply the user's default saved search for this list on load (unless a URL param overrides).
    (function(){ try{ if(cfg.noDefault) return; var _p=new URLSearchParams(location.search); if(_p.get('view')||_p.get('type')) return; fetch('/api/saved-searches?list='+encodeURIComponent(cfg.list),{credentials:'same-origin',cache:'no-store'}).then(function(r){return r.json();}).then(function(j){ if(j&&j.ok&&j.defaultId){ var sc=(j.searches||[]).filter(function(x){return x.id===j.defaultId;})[0]; if(sc){ FILTERS=Object.assign(empty(), sc.payload||{}); if(cfg.onChange) cfg.onChange(); } } }).catch(function(){}); }catch(e){} })();
