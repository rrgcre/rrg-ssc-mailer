...
async function generateBov({ business, files, preparedBy, questionnaire, links, systemPrompt, sdeThreshold, assetSaleFloor }) {
  ...
  try {
    if (state.assetSale === true) {
      ...
      _f.basisOf = "Value is concluded on the net tangible assets that convey with the sale. The business's earnings are insufficient to support a going-concern value, so an asset-sale basis applies.";
