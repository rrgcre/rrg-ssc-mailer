...
- DETERMINISM — NO ANALYST DISCRETION IN THE BRIDGE. Every figure in the bridge must be pulled DIRECTLY from a specific documented number in the financials or the Valuation Questionnaire. Do NOT estimate, round to a "typical" figure, infer, or use judgment about what an add-back "should" be. The same documents must ALWAYS produce the exact same bridge — a second person, or the same person tomorrow, must get identical numbers. Apply these mechanical rules, and if a line's amount is not stated as a specific dollar figure in the documents, set that line to 0 and note the absence in the earnings narrative — never fill it with an assumption:
    ...
    · ownerSalary — the owner's compensation and related payroll tax exactly as documented (officer/owner wages, guaranteed payments) on the P&L, add-back schedule, or VQ. Sum the stated figures; do not impute a salary that is not written down.
    · ownerHealth — only specific, documented owner health / auto / personal expense line items, at their stated amounts. No estimates.
    · familyPayroll — only the exact amount the documents explicitly identify as an above-market or non-working family-payroll add-back. If no specific documented add-back amount exists, use 0. Do NOT estimate a "market wage" to back into this number.
    · oneTime — only items explicitly identified in the financials or VQ as non-recurring, at their stated amounts. If it is not documented as one-time with a figure, it is 0.
    · rentNorm — a mechanical calculation only: (rent actually in the P&L) minus (the market / executed-lease rent from the lease or VQ), both documented figures, expressed as a signed number. If either figure is not documented, use 0.
    · marketGM — the replacement-manager / GM salary as documented in the VQ or financials. If no replacement-labor figure is documented, use 0 and say so in the earnings narrative. Do not invent a market salary.
...
async function generateBov({ business, files, preparedBy, questionnaire, links, systemPrompt, sdeThreshold }) {
  ...
  const resp = await fetch(API_URL, {
    ...
    body: JSON.stringify({ model: MODEL, max_tokens: 8000, temperature: 0, system: sys, messages: [{ role: 'user', content }] }),
  });
  if (!resp.ok) {
